...
use crate::models::{Chat, Message, Poll, MessageType};
...
use crate::ui::scheduled_panel::{SendAtPopover, ScheduledPanel};
...
impl ChatView {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        ...
        // Header bar
        let (header, title_label, status_label, search_btn, call_btn, menu_btn) = Self::create_header();
        ...
        // Input area
        let (input, input_entry, send_btn, voice_btn, attach_btn, emoji_btn, sticker_btn, poll_btn, schedule_btn) =
            Self::create_input();
    ...
    pub fn init_poll_creator(self: &Arc<Self>) {
        ...
        poll_creator.on_submit(move |poll| {
            log::info!("Poll created: {} with {} answers", poll.question, poll.answers.len());
    ...
    fn render_poll_message(&self, msg: &Message, poll: &Poll, parent_box: &GtkBox) {
        ...
        renderer.on_vote(move |_poll_id, answer_ids| {
            ...
            if let (Some(cb), Some(chat_id)) = (on_send.lock().unwrap().as_ref(), chat_id_opt.as_ref()) {
        ...
        self.poll_renderers.lock().unwrap().push((msg.id.clone(), renderer.clone()));
    ...
    fn handle_attach(&self) {
        ...
        glib::spawn_future_local(async move {
            ...
            if let Ok(file) = file_dialog.open_future(None::<&gtk::Window>).await {
                ...
                let bytes = if filename.to_lowercase().ends_with(".jpg") || filename.to_lowercase().ends_with(".png") || filename.to_lowercase().ends_with(".jpeg") {
        ...
        btn_info.connect_clicked(move |_| {
            ...
            if let Some(chat) = cv_info.chat.lock().unwrap().as_ref() {
                ...
                let dialog = gtk::MessageDialog::new(
                    ...
                    gtk::ButtonsType::Ok,
                    &format!("Название: {}\nID: {}\nТип: {:?}", chat.display_name(), chat.id, chat.chat_type)
                );
    ...
    fn edit_last_message(&self) {
        ...
        if let Some(msg) = messages.iter().rev().find(|m| m.from_id == *uid && m.text.is_some()) {
            let msg_text = msg.text.clone().unwrap_or_default();
            
            self.reply_preview_label.set_label(&format!("Редактирование: {}", msg_text));
    ...
    pub fn on_image_open(&self, callback: impl Fn(String, String, Vec<(String, String)>) + 'static) {
    ...
    pub fn set_chat(&self, chat: Chat) {
        *self.chat.lock().unwrap() = Some(chat.clone());
        self.title_label.set_label(chat.title.as_deref().unwrap_or("Чат"));
        ...
        if let Some(_bot_p) = self.bot_panel.lock().unwrap().take() {
            ...
            if let Some(parent) = self.container.parent() {
                ...
                while let Some(current) = child {
                    ...
                    if current.parent().map(|p| p.downcast::<gtk::Box>().map(|box_| {
                        box_.remove(&current);
                        true
    ...
    fn handle_bot_chat(&self, chat: &Chat) {
        ...
        let is_bot = chat.chat_type == crate::models::ChatType::Bot
            || chat.title.as_ref().map(|t| t.contains("Бот")).unwrap_or(false);
...
let on_inline = std::sync::Arc::downgrade(&self.on_inline_click);
    ...
    pub fn init_factory(self: &Arc<Self>) {
        ...
        factory.connect_bind(move |_, list_item_obj| {
            let list_item = list_item_obj.downcast_ref::<gtk::ListItem>().unwrap();
            let item = list_item.item().unwrap().downcast::<crate::ui::message_object::MessageObject>().unwrap();
            ...
            let show_date_sep = if position == 0 {
                ...
                if let Some(prev_item_obj) = this.messages_store.item(position - 1) {
                    if let Ok(prev_item) = prev_item_obj.downcast::<crate::ui::message_object::MessageObject>() {
    ...
    pub fn create_message_row(self: &Arc<Self>, msg: &Message, show_date_sep: bool) -> GtkBox {
        let current_user_id = self.auth.user_id();
        let text = msg.text.as_deref().unwrap_or("[Вложение или системное сообщение]").to_string();
        ...
        let is_bot_msg = self.bot_info.lock().unwrap().as_ref().map(|b| b.bot_id == msg.from_id).unwrap_or(false);
        ...
        let bubble = GtkBox::new(Orientation::Vertical, 4);
            bubble.add_css_class(if is_sent { "bubble-sent" } else { "bubble-received" });
            ...
            right_click_gesture.connect_pressed(move |gesture, _n_press, x, y| {
                ...
                btn_save.connect_clicked(move |button| {
                    ...
                    if let Some(ref cb) = on_save.lock().unwrap().as_ref() {
                        cb(chat_id_save.clone(), msg_id_save.clone(), Some(text_save.clone()));
                    }
            ...
            let has_image = msg.media.iter().any(|m| m.type_ == crate::models::MediaType::Image);
            if has_image {
                let image_media = msg.media.iter()
                    ...
                    let img = gtk::Image::new();
                        ...
                        gesture.connect_pressed(move |_gesture, _n_press, _x, _y| {
                            ...
                            if let Some(cb) = on_image_open.lock().unwrap().as_ref() {
                                cb(url2.clone(), "image.jpg".to_string(), view_clone.get_all_images());
                            }
                        });
            ...
            let has_video = msg.media.iter().any(|m| m.type_ == crate::models::MediaType::Video);
            if has_video {
                let video_url = msg.media.iter()
            ...
            // Voice message
            if let Some(voice_media) = msg.media.iter().find(|m| m.type_ == crate::models::MediaType::Voice) {
            ...
            let time_str = msg.created.with_timezone(&chrono::Local).format("%H:%M").to_string();
    ...
    pub fn show_send_at_popover(&self) {
        ...
        for (name, seconds) in &quick_presets {
            ...
            btn.connect_clicked(move |_| {
                ...
                if let Some(chat_id) = &chat_id_clone {
                    if !text.is_empty() {
                        let utc_dt = chrono::Utc::now() + chrono::Duration::seconds(seconds_clone as i64);
        ...
        confirm_btn.connect_clicked(move |_| {
            ...
            if let Some(chat_id) = &chat_id_confirm {
                if !text.is_empty() {
                    ...
                    if let Some(naive_date) = chrono::NaiveDate::from_ymd_opt(year, month as u32, day as u32) {
                        if let Some(naive_time) = chrono::NaiveTime::from_hms_opt(hour, min, 0) {
                            ...
                            let utc_dt = chrono::TimeZone::from_utc_datetime(&chrono::Utc, &naive_dt);
        ...
        *self.send_at_popover.lock().unwrap() = Some(SendAtPopover::new_with_popover(popover, Some(scheduled_panel)));
        *self.popover_open.lock().unwrap() = true;
        self.schedule_btn.add_css_class("active");
    }

    /// Получить callback для планирования (static helper)
    fn get_schedule_callback(
        chat_id: &str,
        text: &str,
        seconds: u64,
    ) -> Option<Box<dyn Fn()>> {
        ...
        Some(Box::new(move || {
            log::info!("Scheduling message to chat {}: '{}', in {} seconds", chat_id, text, seconds);
        }))
    }
  ...
  fn create_empty_state() -> GtkBox {
    ...
    fn create_header() -> (GtkBox, Label, Label, Button, Button, Button) {
        ...
        let title_label = Label::builder().label("").xalign(0.0).css_classes(vec!["chat-header-title".to_string()]).build();
        let status_label = Label::builder().label("").xalign(0.0).css_classes(vec!["chat-status".to_string()]).build();
        title_box.append(&title_label);
        title_box.append(&status_label);
        
        let search_btn = Button::builder().icon_name("system-search-symbolic").build();
        ...
        (header, title_label, status_label, search_btn, call_btn, menu_btn)
    }

   fn create_input() -> (GtkBox, TextView, Button, Button, Button, Button, Button, Button, Button) {
        ...
        let voice_btn = Button::builder().icon_name("audio-input-microphone-symbolic").build();
        voice_btn.add_css_class("icon-btn");
        voice_btn.add_css_class("voice-btn-premium");
        
        let attach_btn = Button::builder().icon_name("mail-attachment-symbolic").build();
        ...
        input_area.append(&input_pill);
         ...
         (input_area, text_view, send_btn, voice_btn, attach_btn, emoji_btn, sticker_btn, poll_btn, schedule_btn)
    }
}
...
async fn load_inline_image(img: &gtk::Image, url: &str) -> Result<(), String> {
    ...
    if !response.status().is_success() {
        return Err(format!("Inline image fetch failed: HTTP {}", response.status()));
    }
...
fn format_message_text(text: &str) -> String {
    ...
    if let Ok(re_code_block) = regex::Regex::new(r"```([\s\S]+?)```") {
        escaped = re_code_block.replace_all(&escaped, "<tt>$1</tt>").to_string();
    }
    // Inline code
...
fn is_emoji_only(text: &str) -> bool {
    ...
    while let Some(c) = chars.next() {
        ...
        if c == '\u{200D}' || (c >= '\u{FE00}' && c <= '\u{FE0F}') || (c >= '\u{1F3FB}' && c <= '\u{1F3FF}') {
            continue;
        }
